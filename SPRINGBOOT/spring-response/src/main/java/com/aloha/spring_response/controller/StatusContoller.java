...
public class StatusContoller {
    ...
    public ResponseEntity<String> created() {
        ...
        return ResponseEntity.status(HttpStatus.CREATED).body("서버가 새로운 자원을 성공적으로 생성");
    }
    ...
    public ResponseEntity<String> forbidden() {
        ...
        return ResponseEntity.status(HttpStatus.FORBIDDEN).body("클라이언트의 접근이 거부됨(접근 권한 없음)");
    }

    @GetMapping("/404")
    public ResponseEntity<String> notFound() {
        log.info("[GET] - 404 - 자원이 존재하지 않거나, 잘못된 요청경로일 때 상태코드");
        return ResponseEntity.status(HttpStatus.NOT_FOUND).body("자원이 존재하지 않거나, 잘못된 요청경로일 때 상태코드");
    }

    @GetMapping("/500")
    public ResponseEntity<String> internalServerError() {
        log.info("[GET] - 500 - 서버 측 에러나 예외 발생");
        return ResponseEntity.status(HttpStatus.NOT_FOUND).body("서버 측 에러나 예외 발생");
    }
}
